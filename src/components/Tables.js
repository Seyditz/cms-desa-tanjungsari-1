...
import DeleteUserModal from "../pages/components/DeleteUserModal";
import { setDeletedData, showDeleteModal } from "../redux/userListSlice";
import * as postActions from "../redux/postSlice";
import * as umkmActions from "../redux/umkmSlice";
import CustomPagination from "../pages/components/CustomPagination";
import { changePage, setPaginationData } from "../redux/paginationSlice";
import Progress from "./Progress";
import DeletePostModal from "../pages/components/DeletePostModal";
import DeleteUmkmModal from "../pages/components/DeleteUmkmModal";
...
export const UsersTable = () => {
  ...
  const { docs, totalDocs, page, limit } = useSelector(
    (state) => state?.userList
  )?.userList || {docs: []};
  const totalUsers = (page - 1) * limit + docs?.length;
  const loading = useSelector((state) => state?.userList?.loading);
  const loaded = useSelector((state) => state?.userList?.loadingValue);

  const TableRow = (props) => {
    const { username, _id, number } = props;
    const initDeleteModal = () => {
      dispatch(setDeletedData({ _id, username }));
      ...
      <tr>
        ...
        <td>
          <Dropdown as={ButtonGroup}>
            ...
            <Dropdown.Menu>
              <Dropdown.Item href={`/#/users/${_id}`}>
                <FontAwesomeIcon icon={faEye} className="me-2" /> Lihat Detail
              </Dropdown.Item>
              <Dropdown.Item className="text-danger" onClick={initDeleteModal}>
      ...
      <Card.Body className="pt-0">
        <Table hover className="user-table align-items-center">
          ...
          <tbody>
            {!loading ? (
              docs?.map((user, i) => (
                <TableRow key={`user-${user._id}`} {...user} number={((page - 1)* limit) + 1 + i} />
              ))
            ) : (
              <>
                <td></td>
                <td>
                  <Row className="d-flex justify-content-center my-5">
                    <Col className="col-md-5 justify-content-center">
                      <Progress label={"loading"} value={loaded} size={"md"} />
                    </Col>
                  </Row>
                </td>
                <td></td>
        ...
        <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
          ...
          <small className="fw-bold">
            Showing <b>{totalUsers}</b> out of <b>{totalDocs}</b> entries
          </small>
        </Card.Footer>
      </Card.Body>
    </Card>
...
export const PostsTable = () => {
  ...
  const totalPosts = (page - 1) * limit + docs?.length;
  const loading = useSelector((state) => state?.post?.loading);
  const loaded = useSelector((state) => state?.post?.loadingValue);

  const TableRow = (props) => {
    const { name, _id, number, category, image } = props;
    const initDeleteModal = () => {
      dispatch(postActions.setDeletedData({ _id, name, image }));
      dispatch(postActions.showDeleteModal(true));
    };
      ...
      <tr>
        ...
        <td>
          <Dropdown as={ButtonGroup}>
            ...
            <Dropdown.Menu>
              <Dropdown.Item href={`/#/posts/${_id}`}>
                <FontAwesomeIcon icon={faEye} className="me-2" /> Lihat Detail
              </Dropdown.Item>
              <Dropdown.Item href={`/#/update-post/${_id}`}>
                <FontAwesomeIcon icon={faEdit} className="me-2" /> Edit
              </Dropdown.Item>
              <Dropdown.Item className="text-danger" onClick={initDeleteModal}>
                <FontAwesomeIcon icon={faTrashAlt} className="me-2" /> Hapus
  ...
  return (
    <Card border="light" className="table-wrapper table-responsive shadow-sm">
      <Card.Body className="pt-0">
        <Table hover className="post-table align-items-center">
          <thead>
            <tr>
              <th className="border-bottom">No</th>
              <th className="border-bottom">Nama</th>
              <th className="border-bottom">Kategori</th>
              <th className="border-bottom">Aksi</th>
            </tr>
          </thead>
          <tbody>
            {!loading ? (
              docs?.map((post, i) => (
                <TableRow key={`post-${post._id}`} {...post} number={((page - 1)* limit) + 1 + i} />
              ))
            ) : (
              <>
                <td></td>
        ...
        <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
          <CustomPagination loading={loading} />
          <small className="fw-bold">
            Showing <b>{totalPosts}</b> out of <b>{totalDocs}</b> entries
          </small>
        </Card.Footer>
      </Card.Body>
    </Card>
...
export const UmkmTable = () => {
  ...
  const loaded = useSelector((state) => state?.umkm?.loadingValue);

  const TableRow = (props) => {
    const { name, _id, number, category, image } = props;
    const initDeleteModal = () => {
      dispatch(umkmActions.setDeletedData({ _id, name, image }));
      dispatch(umkmActions.showDeleteModal(true));
    };
      ...
      <tr>
        ...
        <td>
          <Dropdown as={ButtonGroup}>
            ...
            <Dropdown.Menu>
              <Dropdown.Item href={`/#/umkm/${_id}`}>
                <FontAwesomeIcon icon={faEye} className="me-2" /> Lihat Detail
              </Dropdown.Item>
              <Dropdown.Item href={`/#/update-umkm/${_id}`}>
                <FontAwesomeIcon icon={faEdit} className="me-2" /> Edit
              </Dropdown.Item>
              <Dropdown.Item className="text-danger" onClick={initDeleteModal}>
                <FontAwesomeIcon icon={faTrashAlt} className="me-2" /> Hapus
      ...
      <Card.Body className="pt-0">
        <Table hover className="post-table align-items-center">
          ...
          <tbody>
            {!loading ? (
              docs?.map((umkm, i) => (
                <TableRow key={`umkm-${umkm._id}`} {...umkm} number={((page - 1)* limit) + 1 + i} />
              ))
            ) : (
              <>
                <td></td>
